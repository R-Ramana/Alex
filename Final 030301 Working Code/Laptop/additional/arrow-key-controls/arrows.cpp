...
void sendData(void *conn, const char *buffer, int len)
{
    ...
    printf("networkActive in sendData: %d\n", networkActive);
    if (networkActive)
    {
...
void *readerThread(void *conn)
{
    char buffer[128];
    int len;
    printf("networkActive in readerThread: %d\n", networkActive);
    while (networkActive)
    {
...
{
    char down, down1, up1, left1, right1;
    arrow = _getch(); //initializing down
    down1 = _getch();
    up1 = _getch();
    left1 = _getch();
    right1 = _getch();
    ...
    while (!quit)
    {
		...
		*/
		...
		else if (ch1 == 'b' || ch1 == 'B') ch = 'b'; // x
		else if (ch1 == 'S' || ch1 == 's') ch = 's';
		else if (ch1 == 'c' || ch1 == 'C') ch = 'c';
		else if (ch1 == 'G' || ch1 == 'g') ch = 'g';
		else if (ch1 == 'q' || ch1 == 'Q') quit = 1;
		else
		{
            cout << ch1;
            ;
...
#define SERVER_NAME "10.42.0.223"
